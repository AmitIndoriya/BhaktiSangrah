...
class AartiDescFragmentNew : Fragment(), View.OnClickListener {
    lateinit var aartiBean: AartiBean3
    ...
    companion object {
        fun newInstance(id: Int, imageid: Int) = AartiDescFragmentNew().apply {
    ...
    override fun onClick(p0: View) {
        when (p0.id) {
            ...
            R.id.next_btn -> {
                (activity as AartiDescActivityNew).goToNextFrag()
            }
            R.id.previous_button -> {
                (activity as AartiDescActivityNew).goToPrevFrag()
            }
        }
    }
